...
k = i - 1
i > 1
d[i][0] = min(d[i - 1][0] + (ka -> ib -> ia 경로의 최소 조작 횟수), d[i - 1][1] + (kb -> ib -> ia 경로의 최소 조작 횟수))
d[i][1] = min(d[i - 1][0] + (ka -> ia -> ib 경로의 최소 조작 횟수), d[i - 1][1] + (kb -> ia -> ib 경로의 최소 조작 횟수))

i = 1
d[i][0] = 시작 위치 -> 1b -> 1a 경로의 최소 조작 횟수
d[i][1] = 시작 위치 -> 1a -> 1b 경로의 최소 조작 횟수

위 처럼 다이나믹 배열을 표현 할수도 있겠지만, 2차원 배열을 사용하는게 더 쉬울것 같아 2차원 배열을 사용함...
```
